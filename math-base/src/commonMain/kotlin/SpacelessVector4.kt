...
/**
 * Represents a [Vector4] without referencing a [Space].
 *
 * @since 0.0.1-alpha.4
 */
@RawSpockApi
@Serializable(SpacelessVector4.Serializer::class)
public data class SpacelessVector4<V>(public val x: V, public val y: V, public val z: V, public val w: V) {
	/**
	 * Creates a proper [Vector4] with the provided [Space].
	 *
	 * @since 0.0.1-alpha.4
	 */
	public infix fun <S> space(space: S): Vector4<V, S> where S : Space<V> = space.vectorOf(x, y, z, w)

	/**
	 * @author Laxystem
	 * @since 0.0.1-alpha.4
	 */
	public class Serializer<V>(private val impl: KSerializer<V>) : KSerializer<SpacelessVector4<V>> {
		@OptIn(InternalSerializationApi::class)
		override val descriptor: SerialDescriptor = buildSerialDescriptor(
			SpacelessVector4::class.qualifiedName!!,
			StructureKind.LIST,
			impl.descriptor
		) {
			...
			element("w", impl.descriptor)
		}

		@OptIn(ExperimentalSerializationApi::class)
		override fun deserialize(decoder: Decoder): SpacelessVector4<V> = decoder.decodeStructure(descriptor) {
			var x: Any? = NotAssignedError
			var y: Any? = NotAssignedError
			var z: Any? = NotAssignedError
			var w: Any? = NotAssignedError
			...
			if (decodeSequentially()) {
				...
				w = decodeSerializableElement(descriptor, 3, impl)
				...
				3 -> w = decodeSerializableElement(descriptor, index, impl)
				DECODE_DONE -> break
				else -> error("did not expect index $index")
			}

			@Suppress("UNCHECKED_CAST")
			SpacelessVector4(x as V, y as V, z as V, w as V)
		}

		override fun serialize(
			encoder: Encoder,
			value: SpacelessVector4<V>
			...
			encodeSerializableElement(descriptor, 3, impl, value.w)
		}
